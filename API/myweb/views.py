...
from django.contrib import auth#django用户（auth)组件一般用在用户的登录注册上，用于判断当前的用户是否合法
...
from . import models
from .models import InterfaceServer, TestCase, TestCaseExecuteResult, CaseSuiteExecuteRecord, CaseSuite, SuiteCase
...
# 用例执行结果菜单项
@login_required
def test_case_execute_records(request):
         ...
         if case_suite_list:
             ...
             for suite_id in case_suite_list:
                ...
                test_suite_record = CaseSuiteExecuteRecord.objects.create(case_suite=test_suite,
                                                                                run_time_interval=count_down_time,creator=username)
                # suite_task(test_suite_record, test_suite, server_address)
                # celery 执行：countdown表示任务延迟的时间，该参数值可以从前端表单中传递过来
...
def case_suite_execute_record(request):
     case_suite_execute_records=models.CaseSuiteExecuteRecord.objects.filter().order_by('-id')
     return render(request, 'case_suite_execute_record.html',
                   {'case_suite_execute_records': get_paginator(request, case_suite_execute_records)})
# 用例集合执行结果-包含用例结果展示
@login_required
...
#登录页的视图函数
def login(request):
    print('request.seeion.items():{}'.format((request.session.items())))
